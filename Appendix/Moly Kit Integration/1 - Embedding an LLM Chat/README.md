...
LLM chat application. You will not need to handle button presses yourself, but
...
trigger and wait the load outselves before displaying it.
...
And we will also use Makepad's `UiRunner` as a concise and pragmatic alternative
to use `Cx::post_action`. An `UiRunner` allows us to send boxed closures from
any thread or async task, back to our widget's scope for execution. The only
requirement to enable it's usage is to add the following to your `handle_event`.
...
into a separate function. I will document what each part does inline with the
code:
